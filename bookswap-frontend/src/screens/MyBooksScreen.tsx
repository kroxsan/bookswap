// BookSwap - MyBooksScreen
// Hafta 10: Kart sol borderi, durum badge renkleri, boş ekran iyileştirmesi
...
const MyBooksScreen = () => {
  ...
  useFocusEffect(useCallback(() => { setLoading(true); fetchMyBooks(); }, []));
  const onRefresh = () => { setRefreshing(true); fetchMyBooks(); };
  const resetForm = () => { setTitle(''); setAuthor(''); setCategory(''); setCondition(''); setDescription(''); };
  ...
  const handleAdd = async () => {
    ...
    setSaving(true);
    const data: CreateBookData = {title: title.trim(), author: author.trim(), category, condition, description: description.trim() || undefined};
  ...
  const handleDelete = (book: Book) => {
    Alert.alert('İlanı Sil', `"${book.title}" ilanını silmek istediğine emin misin?`, [
  ...
  // Hafta 10: iyileştirilmiş kart
  const renderBook = ({item}: {item: Book}) => {
    const isActive = item.status === 'Aktif';
    return (
      <View style={styles.card}>
        {/* Sol çizgi: aktif → yeşil, takaslandı → gri */}
        <View style={[styles.cardBar, {backgroundColor: isActive ? Colors.success : Colors.gray}]} />
          ...
          <View style={styles.cardTop}>
            <View style={styles.cardInfo}>
              ...
              <View style={styles.tags}>
                ...
                <View style={[styles.badge, isActive ? styles.activeBadge : styles.swappedBadge]}>
        ...
        <View style={styles.modalOverlay}>
          <View style={styles.modalContent}>
            ...
            <ScrollView showsVerticalScrollIndicator={false}>
              ...
              <TextInput style={styles.input} placeholder="Kitabın adı" placeholderTextColor={Colors.placeholder} value={title} onChangeText={setTitle} />
              <Text style={styles.label}>Yazar *</Text>
              <TextInput style={styles.input} placeholder="Yazarın adı" placeholderTextColor={Colors.placeholder} value={author} onChangeText={setAuthor} />
              ...
              <TextInput style={[styles.input, styles.textArea]} placeholder="Kitap hakkında kısa bir açıklama..." placeholderTextColor={Colors.placeholder} value={description} onChangeText={setDescription} multiline numberOfLines={3} />
              <TouchableOpacity style={[styles.saveBtn, saving && styles.btnDisabled]} onPress={handleAdd} disabled={saving}>
                {saving ? <ActivityIndicator color={Colors.white} /> : <Text style={styles.saveBtnText}>İlanı Yayınla</Text>}
              </TouchableOpacity>
            </ScrollView>
          </View>
        </View>
...
const styles = StyleSheet.create({
  ...
  header: {backgroundColor: Colors.primary, paddingHorizontal: 20, paddingTop: 14, paddingBottom: 16, flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between'},
  ...
  emptyAddBtn: {backgroundColor: Colors.accent, paddingHorizontal: 24, paddingVertical: 12, borderRadius: 10},
  ...
  badge: {backgroundColor: Colors.primary, paddingHorizontal: 8, paddingVertical: 3, borderRadius: 6},
  ...
  modalContent: {backgroundColor: Colors.white, borderTopLeftRadius: 22, borderTopRightRadius: 22, padding: 24, maxHeight: '90%'},
  modalHandle: {width: 40, height: 4, backgroundColor: Colors.inputBorder, borderRadius: 2, alignSelf: 'center', marginBottom: 16},
  ...
  input: {backgroundColor: Colors.inputBg, borderWidth: 1, borderColor: Colors.inputBorder, borderRadius: 10, paddingHorizontal: 14, paddingVertical: 11, fontSize: 15, color: Colors.primary},
  textArea: {height: 80, textAlignVertical: 'top'},
  chipRow: {flexDirection: 'row', flexWrap: 'wrap', gap: 8},
  chip: {borderWidth: 1, borderColor: Colors.inputBorder, borderRadius: 8, paddingHorizontal: 12, paddingVertical: 7, backgroundColor: Colors.inputBg},
  chipSelected: {backgroundColor: Colors.primary, borderColor: Colors.primary},
  chipText: {fontSize: 13, color: Colors.darkGray},
  chipTextSelected: {color: Colors.white, fontWeight: '600'},
  saveBtn: {backgroundColor: Colors.accent, borderRadius: 10, paddingVertical: 14, alignItems: 'center', marginTop: 20, marginBottom: 8},
