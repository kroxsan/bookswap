// BookSwap - BookDetailScreen — Hafta 8
// isOwner kontrolü getUserId() ile yapılıyor, route'dan currentUserId almıyor
...
import {bookService, Book, getUserId} from '../services/api';
...
const BookDetailScreen = ({route, navigation}: Props) => {
  ...
  useEffect(() => {
    bookService.getById(bookId).then(result => {
      if (result.error) { Alert.alert('Hata', result.error); navigation.goBack(); }
      else setBook(result.data!);
      setLoading(false);
    });
  }, [bookId]);
  ...
  const isOwner = book.userId === getUserId();
      ...
      <ScrollView contentContainerStyle={styles.content}>
        <View style={styles.card}>
          ...
          <View style={styles.tags}>
            <View style={styles.tag}><Text style={styles.tagText}>{book.category}</Text></View>
            <View style={[styles.tag, styles.conditionTag]}><Text style={styles.tagText}>{book.condition}</Text></View>
            <View style={[styles.tag, book.status === 'Aktif' ? styles.activeTag : styles.swappedTag]}><Text style={styles.tagText}>{book.status}</Text></View>
          </View>
          {book.description ? (
        ...
        {!isOwner && book.status === 'Aktif' && (
          <TouchableOpacity style={styles.offerBtn} activeOpacity={0.85}
            onPress={() => navigation.navigate('SendOffer', {targetBook: {id: book.id, title: book.title, author: book.author, userName: book.userName ?? ''}})}>
            <Text style={styles.offerBtnText}>🔄 Takas Teklifi Gönder</Text>
          </TouchableOpacity>
        )}
        {isOwner && (
          <View style={styles.ownerNote}><Text style={styles.ownerNoteText}>Bu ilan sana ait.</Text></View>
        )}
      </ScrollView>
    </SafeAreaView>
  );
...
const styles = StyleSheet.create({
  ...
  header: {backgroundColor: Colors.primary, paddingHorizontal: 16, paddingTop: 12, paddingBottom: 16, flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between'},
  backBtn: {paddingVertical: 4, width: 60},
  backText: {color: Colors.white, fontSize: 15},
  headerTitle: {fontSize: 18, fontWeight: 'bold', color: Colors.white},
  content: {padding: 16, gap: 12},
  card: {backgroundColor: Colors.white, borderRadius: 14, padding: 20, elevation: 2, shadowColor: '#000', shadowOffset: {width: 0, height: 1}, shadowOpacity: 0.08, shadowRadius: 4},
  ...
  offerBtn: {backgroundColor: Colors.accent, borderRadius: 12, paddingVertical: 16, alignItems: 'center', elevation: 2},
  offerBtnText: {color: Colors.white, fontSize: 16, fontWeight: 'bold'},
  ownerNote: {backgroundColor: Colors.inputBg, borderRadius: 10, padding: 14, alignItems: 'center', borderWidth: 1, borderColor: Colors.inputBorder},
